...
double Do_Step(int parity)
{
  ...
  for (x = 1; x < dim[X_DIR] - 1; x++)
    ...
    {
      int global_x = x + offset[X_DIR];
      int global_y = y + offset[Y_DIR];
      if ((global_x + global_y) % 2 == parity && source[x][y] != 1)
        {
            old_phi = phi[x][y];
...
void Solve()
{
  ...
  while (global_delta > precision_goal && count < max_iter)
  {
    ...
    int s;
    for (s = 0; s < sweeps_per_exchange; s++) 
    {
        // Red Sweep
        delta1 = Do_Step(0);
        // Black Sweep
        ...
        // Track the maximum error seen in this batch
        double local_max = (delta1 > delta2) ? delta1 : delta2;
        if (local_max > delta) delta = local_max;
    }
    // --- BATCHED COMPUTATION END ---
    // Now communicate ONCE after 'k' sweeps
    Exchange_Borders();

    // Check convergence ONCE after 'k' sweeps
    MPI_Allreduce(&delta, &global_delta, 1, MPI_DOUBLE, MPI_MAX, grid_comm);

    // Increment count by the actual number of sweeps performed
    count += sweeps_per_exchange;
  }
...
int main(int argc, char **argv)
{
    ...
    // Safety check
    if (sweeps_per_exchange < 1) sweeps_per_exchange = 1;
