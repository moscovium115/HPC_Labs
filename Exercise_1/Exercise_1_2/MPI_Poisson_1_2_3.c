...
double omega = 1.0;		/* relaxation factor for SOR */
...
void Solve()
{
  ...
  while (global_delta > precision_goal && count < max_iter)
  {
    ...
    if (count % 100 == 0 && proc_rank == 0) {
        // We need to peek at the time without resetting it
        // Your current print_timer resets the timer! 
        // Let's manually peek:
        double current_wtime = MPI_Wtime() - wtime; 
        printf("MEASURE: Iterations %d Time %f\n", count, current_wtime);
    }
  }
