...
void Exchange_Borders(double *vect)
{
  ...
  /* Loop over all neighbors of this process */
  for (i = 0; i < N_neighb; i++)
  {
      /* * MPI_Sendrecv exchanges data with one neighbor at a time.
       * * Arguments:
       * 1.  vect, 1, send_type[i] : Send 1 'packet' of boundary data from 'vect'.
       * 2.  proc_neighb[i], 0     : To this neighbor rank, tag 0.
       * 3.  vect, 1, recv_type[i] : Receive 1 'packet' into ghost slots in 'vect'.
       * 4.  proc_neighb[i], 0     : From this neighbor rank, tag 0.
       * 5.  grid_comm, &status    : Use the grid communicator.
       */
...
void Solve()
{
  ...
  /* Timers */
  double t_start;
  double t_calc = 0.0, t_ex = 0.0, t_glob = 0.0, t_idle = 0.0;

  Debug("Solve", 0);

  if ((r = malloc(N_vert * sizeof(double))) == NULL) Debug("Solve : malloc(r) failed", 1);
  if ((p = malloc(N_vert * sizeof(double))) == NULL) Debug("Solve : malloc(p) failed", 1);
  if ((q = malloc(N_vert * sizeof(double))) == NULL) Debug("Solve : malloc(q) failed", 1);

  /* --- Initial Exchange --- */
  t_start = MPI_Wtime();
  MPI_Barrier(grid_comm); /* Sync before comm */
  t_idle += MPI_Wtime() - t_start;

  t_start = MPI_Wtime();
  Exchange_Borders(phi);
  t_ex += MPI_Wtime() - t_start;
  ...
  printf("(%d) Time Breakdown: Calc: %.4fs | Exchange: %.4fs | Global: %.4fs | Idle: %.4fs\n", 
         proc_rank, t_calc, t_ex, t_glob, t_idle);
