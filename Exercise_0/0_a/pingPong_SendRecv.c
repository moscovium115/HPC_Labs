...
int main(int argc, char **argv)
{
    ...
    for (size_t j=0; j<=20; j++)
    {
        ...
        int targetRank = (myRank == 0) ? 1 : 0; // Rank 0 talks to 1, and vice versa
        ...
        if (myRank == 0)
        {
            ...
            for (i=0; i<n_test; i++) 
            {
                // MPI_Sendrecv combines send and receive into one call 
                MPI_Sendrecv(myArray, numberOfElementsToSend, MPI_INT, 1, 0,
                             myArray, numberOfElementsToSend, MPI_INT, 1, 0,
        ...
        else if (myRank == 1)
        {
            ...
            for (i=0; i<n_test; i++)
            {
                MPI_Sendrecv(myArray, numberOfElementsToSend, MPI_INT, 0, 0,
